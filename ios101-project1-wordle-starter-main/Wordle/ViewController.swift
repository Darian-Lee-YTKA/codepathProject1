  ...
  override func viewDidLoad() {
    ...
    keyboardController.didSelectString = { [weak self] selectedString in
        if selectedString == DELETE_KEY {
              // If it's the delete key, call the `deleteLastCharacter` method of `boardController`.
            self?.boardController.deleteLastCharacter()
        } else {
              // If it's not the delete key, call the `enter` method of `boardController` and pass in the selected string.
